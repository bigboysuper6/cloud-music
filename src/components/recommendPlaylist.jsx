...
import DailyRcmCard from "./common/dailyRcmCard/dailyRcmCard";

const RecommendPlaylist = (props) => {
    const [list, setList] = React.useState([]);
    const dailySongsState = useSelector((state) => state.datasongs.value);
    ...
    function getDayRcmCard() {
        if (usrState) {
            console.log("dailySongsState1", dailySongsState);
            return (
                dailySongsState && (
                    <DailyRcmCard dailySongsState={dailySongsState} />
                )
            );
        } else return null;
    }

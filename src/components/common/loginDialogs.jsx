...
import MusicLogo from "../../Image/logo.jpg";
...
export default function LoginDialogs(props) {
  ...
  async function qrlogin() {
    ...
    if (!qruseRef.current) {
      const { data: key_data } = await login.getqrkey();
      const { data: qr_data } = await login.getqrcreate(key_data.data.unikey);
    ...
    let qrlogincheck = setInterval(async () => {
      const { data: code_data } = await login.getqrcheck(keyRef.current);
      ...
      if (code_data.code === 803) {
        setprompt("授权登录成功");
        await login.getloginstatus(code_data.cookie);
